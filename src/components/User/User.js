...
export default function Member() {
    ...
    useEffect(() => {
        async function getMembers() {
            ...
            const res = await axios.get(`${httpRequest.api.baseUrl}/member?page=${page}`);
            if (res.status === 200) {
    ...
    const handleSearch = async e => {
        ...
        const res = await axios.get(`${httpRequest.api.baseUrl}/member?page=${page}&&search=${e.target.value}`);
    ...
    return (
        <div className="flex-grow w-full px-3 md:px-6 pt-10 md:mt-0 md:ml-64 pb-5">
            <TopNavbar path=" Anggota" title="Daftar Anggota" className="bg-slate-50"></TopNavbar>

            <div className="member__container bg-white rounded-lg shadow-lg">
                <div className="table_head__container flex justify-between p-5 box-border items-center">
                    <div className="flex w-72 h-full">
                        <input id="keyword__input" placeholder="Ketik nama" onInput={handleSearch} className="font-sans focus:outline-none border-l-2 border-y-2 w-full h-5 rounded-l-full p-5 " type="text"></input>
                        ...
                        <tbody>
                            {
                                members && members?.length < 1 ?
                                    ...
                                    : members.map(member => {
                                        return (
                                            <tr className="border text-left hover:bg-slate-50">
